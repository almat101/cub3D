...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/30 13:02:03 by amatta            #+#    #+#             */
/*   Updated: 2024/01/30 13:12:56 by amatta           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_textwall(t_data *data, t_game *cube, int x)
{
	...
	data->tex_pos = (data->draw_start - SH / 2 + data->line_height / 2)
		* data->step;
	y = data->draw_start;
	...
	if (data->side == 1)
	{
		if (data->ray_dir_y > 0)
			data->color = *(unsigned int *)
				(cube->card->north_wall.addr + shift);
		else
			data->color = *(unsigned int *)
				(cube->card->south_wall.addr + shift);
	}
	else
	{
		if (data->ray_dir_x > 0)
			data->color = *(unsigned int *)
				(cube->card->east_wall.addr + shift);
		else
			data->color = *(unsigned int *)
				(cube->card->west_wall.addr + shift);
	}
}
...
int	wall_pos(t_data *data, t_game *cube)
{
	...
	data->line_height = (int)(SH / data->perp_wall_dist);
	data->draw_start = -data->line_height / 2 + SH / 2;
	if (data->draw_start < 0)
		data->draw_start = 0;
	data->draw_end = data->line_height / 2 + SH / 2;
	if (data->draw_end >= SH)
		data->draw_end = SH - 1;
