...
void	check_map(t_game *cube)
{
	...
	while (cube->real_map[y])
	{
		...
		while (cube->real_map[y][x])
		{
			if (ft_strchr("0NWSE", cube->real_map[y][x]))
			{
				if ((cube->real_map[y - 1][x] == 32 || cube->real_map[y + 1][x] == 32 || cube->real_map[y][x - 1] == 32 || cube->real_map[y][x + 1] == 32 ) || (cube->real_map[y][x + 1] == '\n' || cube->real_map[y][x - 1] == '\n' || cube->real_map[y + 1][x] == '\n' || cube->real_map[y - 1][x] == '\n') || (cube->real_map[y][x] == '0' && cube->real_map[y + 1 ][x] == '1' && (cube->real_map[y + 1][x + 1] == '\n' || cube->real_map[y + 1][x - 1] == 32)))
					free_err(cube, "Error\n");
			}
			x++;
		}
